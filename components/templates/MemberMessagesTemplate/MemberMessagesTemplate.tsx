  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.url) {
        ...
        // We'll optionally use inputMsg if the user had typed something, but sending media usually goes directly.
        await onSendMessage(inputMsg.trim() || "Sent an attachment", json.url, mediaType);
        setInputMsg("");
      }
    } catch (err) {
      console.error("Failed to upload media in chat:", err);
      ...
      <div className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden bg-[#F1F4F2]">
        ...
        <header className="bg-white border-b border-[#E9EDEA] px-4 md:px-7 py-3 md:py-4 flex items-center justify-between shrink-0 shadow-xs z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[17px] font-extrabold text-[#1B2321]">{groupName || "Loading..."}</div>
        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-7 flex flex-col gap-4 pb-24 md:pb-20">
                      ...
                      <div className="mb-2">
                        {msg.mediaType === 'image' ? (
                          ...
                          <a 
                            ...
                            className="flex items-center gap-2 underline text-emerald-100 hover:text-white"
                          >
          ...
          {isUploadingMedia && (
            <div className="flex justify-center text-xs text-[#5B6B65]">
              Uploading file...
            </div>
          )}
        </main>
        ...
        <footer className="bg-[#F1F4F2] p-2 md:p-3 flex items-end gap-2 shrink-0 z-10 relative mb-[82px] md:mb-0">
          ...
          <div className="flex-1 bg-white rounded-[22px] min-h-[44px] flex items-end px-2 py-1 shadow-sm border border-[#E9EDEA] transition-all">
            ...
            <div className="flex items-center gap-1 shrink-0 pr-1 mb-0.5">
              <button 
                ...
                className="w-9 h-9 rounded-full flex items-center justify-center text-[#94A29C] hover:text-[#5B6B65] transition-colors relative overflow-hidden"
          ...
          <button 
            ...
            className={`w-[46px] h-[46px] rounded-full bg-[#2D7A52] text-white flex items-center justify-center shrink-0 shadow-sm transition-transform mb-0.5 ${(!inputMsg.trim() && !isUploadingMedia) ? 'opacity-50' : 'active:scale-95'}`}
          >
            {inputMsg.trim() ? (
